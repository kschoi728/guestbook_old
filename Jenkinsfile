...
pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/kschoi728/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/kschoi728/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://kschoi728:${GITHUB_ACCESS_TOKEN}@github.com/kschoi728/guestbook-config.git"
                    '''
                }
            }
        }
